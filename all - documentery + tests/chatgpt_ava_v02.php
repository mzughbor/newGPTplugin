...
Author: mZughbor
*/
define('CUSTOM_LOG_PATH', WP_CONTENT_DIR . '/chatgpt_ava/log.txt');
    ...
    if (is_wp_error($response)) {
        ...
        */
        if (is_array($response_body) && isset($response_body['choices'][0]['message']['content'])) {
            ...
            error_log('chatgpt_ava_send_message function >> ChatGPT API Response Error: ' . print_r($response_body, true), 3, CUSTOM_LOG_PATH);
            $output = 'Generated content from ChatGPT';
        }
...
function chatgpt_ava_private_rewrite()
{
    ...
    */
    ...
    function generate_content_with_min_word_count($filtered_content, $api_key)
    {
        ...
        if (is_wp_error($response)) {
            // Log the error for debugging
            error_log('ChatGPT API Error: ' . $response->get_error_message(), 3, CUSTOM_LOG_PATH);
        ...
        if (isset($response_body['choices']) && is_array($response_body['choices']) && !empty($response_body['choices'])) {
            ...
            error_log('ChatGPT API Response Error: ' . print_r($response_body, true), 3, CUSTOM_LOG_PATH);
    ...
    foreach ($private_posts as $post) { 
        ...
        // Rest of the code to generate and update content based on the API response...
            ...
            $message = "Rewrite this article {$post_content}, covering it to become less than 400 words in total using the Arabic language. Structure the article with clear headings enclosed within the appropriate heading tags (e.g., <h1>, <h2>, etc.) and generate subtopics inside the article to use subheadings, each one of them should have at least one paragraph. Use a cohesive structure to ensure smooth transitions between ideas, focus on summarizing and shortening the content, and make sure it's at least not less than 300 words. Make it coherent and proficient. Remember to (1) enclose headings in the specified heading tags to make parsing the content easier. (2) Wrap even paragraphs in <p> tags for improved readability. (3) make sure that 25% of the sentences you write contain less than 20 words.";
            ...
            if ($word_count == 0){
                ...
                while ($word_count <= $min_word_count) {
                    ...
                    if (!empty($found_words)) {
                        ...
                        // for debugging
                        error_log('The response contains the following words: ' . print_r(implode(', ', $found_words), true), 3, CUSTOM_LOG_PATH);
                        ...
                        error_log('The response does not contain any of the specified words: ', true, 3, CUSTOM_LOG_PATH);
                    ...
                    if ($response === '<p>yes</p>' or $response === '<p>yes.</p>') {
                        ...
                        if (!empty($found_words)) {
                            // Some of the words were found in the $response
                            // for debugging
                            error_log('The response contains the following words: 2' . print_r(implode(', ', $found_words), true), 3, CUSTOM_LOG_PATH);
                             // Log the response for debugging  
                            ...
                            // None of the words were found in the $response
                            error_log('The response does not contain any of the specified words: 2', true, 3, CUSTOM_LOG_PATH);
            ...
            elseif ($word_count > $min_word_count) {
                ...
                // Log the error for debugging
                error_log('Content generation failed for private post ID: ' . $post->ID, 3, CUSTOM_LOG_PATH);
